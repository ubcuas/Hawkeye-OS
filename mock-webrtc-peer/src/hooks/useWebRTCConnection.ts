...
export function useWebRTCConnection(): UseWebRTCConnectionResult {
  const [signalingStatus, setSignalingStatus] =
    useState<SignalingStatus>("disconnected")
      ...
      if (event.streams && event.streams[0]) {
        ...
        stream.addEventListener("addtrack", (e) => {
          console.log(
            ...
            (e as MediaStreamTrackEvent).track.kind,
          )
        })

        stream.addEventListener("removetrack", (e) => {
          console.log(
            "Track removed from stream:",
            (e as MediaStreamTrackEvent).track.kind,
          )
        })
    ...
    try {
      ...
      if (peerConnectionRef.current) {
        ...
        if (currentState !== "stable") {
          console.warn(
            "Rejecting offer - peer connection not in stable state:",
            currentState,
          )
          return
        }
