...
# Camera intrinsics - replace with your real calibrated values
FX = 600.0
FY = 600.0
CX = 320.0
CY = 240.0
...
# Camera position offsets relative to FC (meters) TODO: REPLACE WITH ACTUAL VALUES
...
class DetectionBridge(Node):
    ...
    def detection_cb(self, msg: TaggedImage):
        ...
        if len(msg.bounding_box) < 2:
            self.get_logger().warn("Ignoring detection: bounding_box has fewer than 2 points")
            return
        ...
        try:
            ...
            if depth_m is None:
                self.get_logger().warn("Ignoring detection: no valid depth near bbox center")
                return
    ...
    def decode_depth_image_to_meters(self, depth_msg):
        ...
        if depth_img is None:
            raise ValueError(f"Failed to decode depth image, format='{depth_msg.format}'")


        if depth_img.dtype == np.uint16:
            # common case: millimeters
